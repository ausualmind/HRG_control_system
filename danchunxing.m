...
b=[40;20;12];
c=[-3,-5,0,0,0];
[B, x, baseB, baseD] = Simplex(A,b,c,[3,4,5],100000);
function [B, x, baseB, baseD] = Simplex(A,b,c,bases,iterations)
...
% bases, index vector for column basis vectors
    ...
    [B, x, baseB, baseD] = Simplex1(A, eye(m), b, c, baseB, baseD,iterations);
end
...
%             println("Not bounded!");
        ...
        B = PivotalTransform(horzcat(B,x,xq),p, m+2);
        x = B(:, m+1);
        B = B(:, 1:m);
%         println("B = ",B);
        temp =baseB(p);
