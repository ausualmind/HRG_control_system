...
for k = 1: inref.noP
    ...
    %% 新加创新点
    Swarm.Particles(k).V_j3 = rand(1,nVar); % 随机数
    Swarm.Particles(k).PBEST.X_j3 = rand(1,nVar).*inref.Vmax; % 随机数
...
%         ADRC.beta01=Swarm.Particles(k).X(1); ADRC.beta03=Swarm.Particles(k).X(2);
    ...
    %% 判断双目标最优粒子位置在从前3代开始是否越来越远？/若是，则双目标较优粒子杂交变异
    ...
    %% 判断全局最优位置在5代以内是否有变动？/若无，则PSO杂交变异
    ...
    %% 新加创新点/判断全局最优位置在5代以内是否有变动？/若无，则PSO杂交变异
    ...
    for k=1:inref.noP % 适应度函数值计算
        ...
        Swarm.Particles(k).X = Swarm.Particles(k).X + Swarm.Particles(k).V;%适应度函数公式2计算
        ...
        Swarm.Particles(k).X_j3 = Swarm.Particles(k).X_j3 + Swarm.Particles(k).V_j3;%适应度函数公式2计算
